...
import org.tcloud.repository.ArticleDao;
import org.tcloud.repository.CategoryDao;
...
public class ArticleService {
	...
	public Article getArticleFromArticleInfo(ArticleInfo articleInfo) {
		...
		for (int i = 0; i < newStrings.length; i++) {
			...
			if (SetOpt.exist(list, escapeString, Tag.class)) {
				t = tagDao.findOneByName(escapeString);
			}
			tags.add(t);
		}
	...
	public Page<Article> findLatestArticles(int pageIndex, int pageSize) {
		return articleDao.findAllByCreatedDateDesc(
	...
	public Page<Article> findMostPopArticles(int pageIndex, int pageSize) {
		return articleDao.findAllByPVDesc(
	...
	public Page<Article> findArticlesFromCategory(Category category,
			int pageIndex, int pageSize) {
		return articleDao.findByCategoryOrderByCreatedDateDesc(
				category.getEnName(),
				constructCreatedDatePageSpecification(pageIndex, pageSize));
	}
	...
	public Page<Article> findArticlesFromTag(Tag tag, int pageIndex,
			int pageSize) {
		return articleDao.findByTagsOrderByCreatedDateDesc(tag.getTagValue(),
				constructCreatedDatePageSpecification(pageIndex, pageSize));
	}
	...
	public HashMap<String, Article> findPrevAndNextArticle(Article article) {
		...
		List<Article> prevArticle = articleDao.findPrevArticle(article
				.getCreatedDate());
		List<Article> nextArticle = articleDao.findNextArticle(article
				.getCreatedDate());
	...
	public List<HashMap<String, Object>> findArticlesInSameTag(Article article,
		...
		for (Article m : articleDao.findArticlesInSameTag(article.getId(),
				constructCreatedDatePageSpecification(pageIndex, pageSize))
				.getContent()) {
